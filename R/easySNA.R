...
keywordsSNA <- function(docs, support=0.1, confidence=0.8)
{
  ...
  dtm <- makeDTM(docs.df, key=keywords , TEXT.name = "docs", RHINO = TRUE)
  dtm.df <- as.data.frame(dtm)		  # 행렬 결과를 데이터프레임으로 변환
  dtm.abovemean <- ifelse(dtm > 0, 1, 0)  # 값이 있으면 1, 없으면 0

  library(arules)
  rules <- apriori(dtm.abovemean, parameter = list(supp=support, conf=confidence))

  library(arulesViz)
  plot(rules, method = "graph", nodeCol="blue", labelCol="red")
